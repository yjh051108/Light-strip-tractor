...
struct DetectionParams {
    ...
    int brightness_threshold = 240;  // 从100增加到200
    int bilateral_d = 7;
...
private:
    ...
    void showPreprocessing(const Mat& frame, const Mat& gray, const Mat& binary) {
        // 设置较小的显示尺寸
        const Size displaySize(320, 180);  // 16:9比例的小尺寸
        int width = displaySize.width/3;
        int height = displaySize.height/2;
...
public:
    ...
    void showDebugInfo(const Mat& frame, const vector<LightBar>& lights) {
        ...
        // 缩放显示
        Mat small_debug;
        resize(debug_img, small_debug, Size(480, 270));  // 16:9比例
        imshow("Debug Window", small_debug);
    }
    ...
    vector<LightBar> detect(Mat& frame) {
        ...
        for(const auto& c : contours) {
            try {
                ...
                // 计算灯条的倾斜角度（相对于y轴）
                float angle = r.angle;
                if(r.size.width > r.size.height) {  // OpenCV的角度需要根据长宽进行调整
                    angle = angle + 90;
                }
                angle = abs(angle);  // 取绝对值，因为我们关心的是偏离y轴的角度
                
                // 判断角度是否在±45度范围内
                if(angle > 45 && angle < 135) {
                    continue;  // 如果倾斜角度超过45度，跳过这个轮廓
                }
                
                // 只有当长宽比和角度都符合要求时，才认为是灯带
        ...
        // 显示所有矩形的调试窗口时进行缩放
        Mat small_rect_debug;
        resize(all_rect_debug, small_rect_debug, Size(480, 270));
        imshow("All Rectangles", small_rect_debug);
        return lights;
    }
};
...
int main() {
    ...
    while(true) {
        if(!cap.read(frame)) {
            cap.set(CAP_PROP_POS_FRAMES, 0);  // 视频结束时重新开始
            continue;
        }
        ...
        else if(key == 32) {  // 空格暂停/继续
            while(true) {
                char k = waitKey(0);
                if(k == 32) break;  // 再次按空格继续
                else if(k == 27) return 0;  // ESC退出
            }
        }
        else if(key == 's') {
